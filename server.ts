...
<footer>made by Jonas Lee<a onclick="openCM()">manage storage</a></footer>
...
_r();_s();
var _km={'favorites':'hub favorites','ojjychess_token':'ojjyChess','CookieClickerGame':'Cookie Clicker','CookieClickerGameBeta':'Cookie Clicker (beta)','CookieClickerGameOld':'Cookie Clicker (old)','CookieClickerGamev10466':'Cookie Clicker (v1)'};
function _gn(k){if(_km[k])return _km[k];if(k.startsWith('_ts_'))return null;return k}
...
function _scan(){var items={},total=0;for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);var n=_gn(k);if(!n)continue;var v=localStorage.getItem(k)||'';var sz=k.length+v.length;var ts=localStorage.getItem('_ts_'+k);ts=ts?parseInt(ts):null;if(!items[n])items[n]={name:n,keys:[],size:0,lastSaved:null};items[n].keys.push(k);items[n].size+=sz;if(ts&&(!items[n].lastSaved||ts>items[n].lastSaved))items[n].lastSaved=ts;total+=sz}return{items:Object.values(items).sort(function(a,b){return b.size-a.size}),total:total}}
function _scanCaches(cb){if(!window.caches){cb([]);return}caches.keys().then(function(names){cb(names.map(function(n){return{name:'cache: '+n,keys:[n],size:0,lastSaved:null,isCache:true}}))}).catch(function(){cb([])})}
function _scanIDB(cb){if(!indexedDB.databases){cb([]);return}indexedDB.databases().then(function(dbs){cb(dbs.map(function(db){return{name:'indexeddb: '+db.name,keys:[db.name],size:0,lastSaved:null,isIDB:true}}))}).catch(function(){cb([])})}
function renderCM(){var d=_scan();_scanCaches(function(caches){_scanIDB(function(idbs){var all=d.items.concat(caches).concat(idbs);var el=document.getElementById('cm');var h='<div class="cm-hd"><h2>storage manager</h2><button onclick="closeCM()">&times;</button></div>';h+='<div class="cm-sum">'+_sz(d.total)+' in localStorage across '+d.items.length+' items';if(caches.length)h+=' + '+caches.length+' browser cache'+(caches.length>1?'s':'');if(idbs.length)h+=' + '+idbs.length+' database'+(idbs.length>1?'s':'');h+='</div>';if(all.length===0){h+='<div class="cm-empty">no cached data found</div>';el.innerHTML=h;return}all.forEach(function(it,i){h+='<div class="cm-it"><div class="cm-it-hd"><span class="cm-it-name">'+it.name+'</span><button class="cm-it-btn" onclick="clearItem('+i+')">clear</button></div><div class="cm-it-meta">';if(!it.isCache&&!it.isIDB)h+='<span>'+_sz(it.size)+'</span>';h+='<span>'+_ago(it.lastSaved)+'</span>';if(it.keys.length>1)h+='<span>'+it.keys.length+' keys</span>';h+='</div>';if(it.keys.length>0&&!it.isCache&&!it.isIDB){h+='<div class="cm-it-keys">'+it.keys.join(', ')+'</div>'}h+='</div>'});h+='<hr class="cm-sep"><div class="cm-da"><button onclick="clearAll()">clear all data</button></div>';el.innerHTML=h;window._cmItems=all})})}
