...
var _cim={'_C2SaveStates':'OvO (saves)','c3offline-ojjygamehub':'Fall Guys (offline)','IDBFS':'Space Waves','localforage':'OvO (data)','firebaseLocalStorageDb':'Rocket Goal IO'};
var _cip=[['eagler','ojjyClient'],['_EaS','ojjyClient'],['EaglerSP','ojjyClient']];
function _cin(n){if(_cim[n])return _cim[n];for(var i=0;i<_cip.length;i++){if(n.indexOf(_cip[i][0])>=0)return _cip[i][1]}return n}
function _scanCaches(cb){if(!window.caches){cb([]);return}caches.keys().then(function(names){cb(names.map(function(n){return{name:'cache: '+_cin(n),keys:[n],size:0,lastSaved:null,isCache:true}}))}).catch(function(){cb([])})}
function _scanIDB(cb){if(!indexedDB.databases){cb([]);return}indexedDB.databases().then(function(dbs){cb(dbs.map(function(db){return{name:'database: '+_cin(db.name),keys:[db.name],size:0,lastSaved:null,isIDB:true}}))}).catch(function(){cb([])})}
